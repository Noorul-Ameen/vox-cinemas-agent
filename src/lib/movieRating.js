...
const CERTIFICATE_PATTERN = /\b(?:age\s*(?:rating|classification|certificate|restriction|limit)|parental\s+guidance|certificate|certification|classified|underage|child|children|kid|kids|year[ -]?old|years?\s+old|y\/?o|take\s+my|bring\s+my|can\s+my\s+(?:son|daughter|child|kid)|suitable\s+for|appropriate\s+for|rated\s+(?:g|pg|1[58]|21)|pg[ -]?(?:13|15)|(?:15|18|21)\s*(?:\+|plus)|(?:fifteen|eighteen|twenty\s+one)\s+plus|18\s*tc|eighteen\s+t\s*c)\b|(?:تصنيف\s*عمري|التصنيف\s*العمري|شهادة\s*عمرية|حد\s*العمر|قيود\s*العمر|إرشاد\s*أبوي|ارشاد\s*ابوي|طفل|أطفال|اطفال|ابني|إبني|بنتي|ابنتي|اصطحب|آخذ|اخذ|مناسب\s*(?:للأطفال|للاطفال)|فوق\s*(?:15|18|21)|بي\s*جي\s*(?:13|15))/iu;
const REVIEW_PATTERN = /\b(?:review|reviews|reviewed|score|stars?|imdb|rotten\s+tomatoes|metacritic|critics?|audience\s+score|box\s+office|out\s+of\s+(?:five|ten|5|10)|worth\s+(?:watching|seeing)|good\s+(?:movie|film|rating)|bad\s+(?:movie|film|rating))\b|(?:مراجعة|مراجعات|تقييم\s*(?:النقاد|الجمهور|آي\s*إم\s*دي\s*بي|imdb)|نقاط|نجوم|من\s*(?:5|10|خمسة|عشرة)|يستحق\s*المشاهدة|رأيك\s*في|رايك\s*في)/iu;
const GENERIC_RATING_PATTERN = /\b(?:rating|rated|classification)\b|(?:التقييم|تقييم|التصنيف|مصنف)/iu;
