...
export default function BookingQRCode({ booking, size = 132 }) {
  ...
  const isPrototype = booking?.verified !== true
    || booking?.demo === true
  ...
  const qrValue = isPrototype ? ref : providerQrValue;
  if (!qrValue) {
    return (
  ...
  const hint = t(isPrototype ? "booking.qrDemoHint" : "booking.qrHint");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7, borderTop: "1px dashed rgba(255,255,255,.15)", padding: "15px 20px" }}>
      <div data-qr-value={qrValue} aria-label={`${hint}: ${ref}`} style={{ display: "grid", placeItems: "center", borderRadius: 12, background: "#fff", padding: 9 }}>
        <QRCodeSVG value={qrValue} size={size} level="M" marginSize={0} title={isPrototype ? `${t("booking.ref")} ${ref}` : hint} />
      </div>
      <div style={{ maxWidth: 260, fontSize: 10, lineHeight: 1.4, color: isPrototype ? "#FFCF70" : "rgba(255,255,255,.48)", textAlign: "center" }}>{hint}</div>
    </div>
  );
}
