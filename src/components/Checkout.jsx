...
export default function Checkout({ order, onPaid, onCancel }) {
  ...
  const pay = (method, label) => {
    setPaying(method);
    setTimeout(() => { setDone(true); setTimeout(() => onPaid({ method, label }), 700); }, 1600);
  };

  const addCard = () => {
    const num = form.num.replace(/\s/g, "");
    if (!luhnOk(num)) return setErr("Card number invalid — try test card 4111 1111 1111 1111");
    if (!/^\d{2}\/\d{2}$/.test(form.exp)) return setErr("Expiry must be MM/YY");
    if (!/^\d{3,4}$/.test(form.cvv)) return setErr("CVV must be 3-4 digits");
    if (!form.name.trim()) return setErr("Name on card required");
  ...
  if (paying) {
    const label = paying === "apple" ? "Apple Pay" : paying === "samsung" ? "Samsung Pay" : "Card payment";
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 320, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>{done ? "Payment approved" : `Authorizing ${label}…`}</div>
        <div style={{ marginTop: 6, fontSize: 12, color: "rgba(255,255,255,.5)" }}>
          {done ? "Confirming your booking" : "Simulated authorization — demo only"}
        </div>
        <div style={{ marginTop: 14, fontSize: 22, fontWeight: 800, color: "#fff" }}>AED {order.total}</div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        <button onClick={onCancel} style={{ background: "none", border: "none", color: "rgba(255,255,255,.5)", cursor: "pointer", padding: 4 }}><ChevronLeft size={18} /></button>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>Checkout</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)" }}>{order.movieTitle} · {order.showtime} · Seats {order.seats.join(", ")}</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderRadius: 12, border: "1px solid rgba(255,255,255,.1)", background: "rgba(0,0,0,.25)", padding: "12px 14px", marginBottom: 16 }}>
        <span style={{ fontSize: 13, color: "rgba(255,255,255,.6)" }}>{order.seats.length} seat{order.seats.length > 1 ? "s" : ""} · {order.screen}</span>
        <span style={{ fontSize: 18, fontWeight: 800, color: "#fff" }}>AED {order.total}</span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
        ...
        <button onClick={() => pay("samsung", "Samsung Pay")} style={{ ...walletBtn, background: "#1428A0" }}>
          <Smartphone size={15} style={{ marginRight: 6 }} /> Samsung Pay
        </button>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "2px 0 12px", color: "rgba(255,255,255,.35)", fontSize: 11 }}>
        <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,.12)" }} /> or pay with card <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,.12)" }} />
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {cards.map((c) => (
            <button key={c.id} onClick={() => setSelected(c.id)}
              style={{ display: "flex", alignItems: "center", gap: 12, borderRadius: 12, padding: "12px 14px", cursor: "pointer", textAlign: "left",
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: "rgba(255,255,255,.45)" }}>{c.name} · {c.exp}</div>
              </div>
      ...
      {adding && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <input value={form.num} onChange={(e) => setForm({ ...form, num: fmtNum(e.target.value) })} placeholder="Card number (try 4111 1111 1111 1111)" style={inp} inputMode="numeric" />
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Name on card" style={inp} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <input value={form.exp} onChange={(e) => setForm({ ...form, exp: e.target.value.replace(/[^\d/]/g, "").slice(0, 5) })} placeholder="MM/YY" style={inp} inputMode="numeric" />
            <input value={form.cvv} onChange={(e) => setForm({ ...form, cvv: e.target.value.replace(/\D/g, "").slice(0, 4) })} placeholder="CVV" style={inp} inputMode="numeric" type="password" />
          </div>
          {err && <div style={{ fontSize: 12, color: "#D9556B" }}>{err}</div>}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <button onClick={() => { setAdding(false); setErr(""); }} style={{ ...actionBtn, background: "rgba(255,255,255,.08)" }}>Cancel</button>
            <button onClick={addCard} style={{ ...actionBtn, background: C.purple }}>Save card</button>
          </div>
        </div>
      )}
      ...
      {!adding && (
        <button disabled={!selected} onClick={() => { const c = cards.find(x => x.id === selected); pay("card", `${c.brand} •••• ${c.last4}`); }}
          ...
          <Lock size={14} /> Pay AED {order.total}
        </button>
      )}

      <div style={{ marginTop: 10, textAlign: "center", fontSize: 10, color: "rgba(255,255,255,.35)" }}>
        Demo checkout — no real payment is processed. Card details never leave this screen.
      </div>
    </div>
  );
}
...
const inp = { background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 10, padding: "11px 13px", color: "#fff", fontSize: 13, outline: "none", width: "100%", boxSizing: "border-box" };
