...
assert.match(finalizeSeats, /Number\(ticketQuantityRef\.current\) === expectedQuantity[\s\S]*sameSeatSelection\(seatsRef\.current, valid\)/, "a stale quote must not commit changed seats or quantity");
...
assert.match(touchSeatConfirmation, /const backFromSeatMap[\s\S]*planContextRef\.current = null[\s\S]*showStage\(\{ view: "showtimes"/, "seat-map Back must synchronously invalidate pricing and leave the seat map");
