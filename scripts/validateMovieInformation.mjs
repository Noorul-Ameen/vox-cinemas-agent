...
assert.match(appSource, /rememberedMovieIsRelevant/);
assert.match(appSource, /visibleMovies\.some\(\(movie\) => String\(movie\?\.id \|\| movie\?\.title\) === String\(rememberedMovie\?\.id \|\| rememberedMovie\?\.title\)\)/);
assert.match(appSource, /visibleMovies\.length === 1 \? visibleMovies\[0\] : null/);
